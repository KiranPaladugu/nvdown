...
public class ReadLightNovelWebDownloader {
                ...
                if (pEleemnts != null && pEleemnts.size() > 0) {
                    ...
                    for (Element aElement : aElements) {
                        ...
                        System.out.println(String.format(
                                "Found download link for [%s], [%s],[%s] at link [%s]", name,
    ...
    private void download(boolean download, String path, String name, NovelData data,
            String contentLink, String id) {
        File file = new File(path + name + "_" + id + ".xhtml");
        String tmpLink = contentLink;
        if (data.isLinkDownloaded(contentLink)
                || data.isLinkDownloaded(tmpLink.replace("https:", "http"))
                || file.exists()) {
            ...
            if (downloadLinkContent(getUrlContents(contentLink, download), path,
                    name, id, download)) {
                data.addDownloadedLink(contentLink);
            }
        }
    ...
    private static boolean copyToPool(File sourcePath, File fileToCopy) {
        if (fileToCopy != null && fileToCopy.exists()) {
            try {
                ...
                dest = Files.copy(FileSystems.getDefault().getPath(fileToCopy.toString()),
                        dest.resolve(fileToCopy.getName()), StandardCopyOption.REPLACE_EXISTING);
                if (dest != null)
                    return true;
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static String getUrlContents(String theUrl, boolean download) {
        ...
        try {
            ...
            if (redirect) {
                ...
                urlConn = (HttpURLConnection) new URL(urlConn.getHeaderField("Location"))
                        .openConnection();
            }
    ...
    private boolean downloadLinkContent(String htmlString, String targetDir, String targetFileName,
            String targetFileId, boolean append) {
        if (!isDownload()) {
            return true;
        }
        ...
        if (content != null) {
            ...
            if (count > 0) {
                ...
                System.out.println(
                        String.format("Successfully downloaded content for [%s] with id [%s]",
                                targetFileName, targetFileId));
                result = true;
            } else {
                System.err
                        .println(String.format("Failed to downloaded content for [%s] with id [%s]",
                                targetFileName, targetFileId));
            }

        } else {
            System.err.println(String.format("No Content available for [%s] wiht id [%s]",
                    targetFileName, targetFileId));
        }
        return result;
    }
